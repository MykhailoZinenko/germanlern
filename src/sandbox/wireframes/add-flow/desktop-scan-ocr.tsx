/**
 * @screen add-flow/desktop-scan-ocr
 * @shell DesktopShell with scrim on content only
 *
 * Desktop add word modal — Scan tab, OCR running state.
...
import { ScanOcrRunningContent } from "./_tab-contents";

export function AWDesktopScanOcr() {
  return (
    <DesktopShell
      active="library"
    ...
    >
      <div style={{ position: "absolute", inset: 0, background: "rgba(42,26,16,0.28)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10 }}>
        <DesktopAddModal tab="scan" bufferCount={0}>
          <ScanOcrRunningContent />
        </DesktopAddModal>
      </div>
    </DesktopShell>
  );
