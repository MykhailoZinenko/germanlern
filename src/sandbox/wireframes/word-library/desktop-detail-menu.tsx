/**
 * @screen word-library/desktop-detail-menu
 * @shell DesktopShell active=library, showCompanionMini=false, contentScroll=false
 * @add-ref ADD.md §12 · Word Library » Detail » More menu
 *
 * Same layout as desktop-detail-list, but the "more" dropdown is open
 * over the WordDetail action row (Reset progress / Mark as leech / Archive
 * / Delete).
 *
 * In production this is a shadcn DropdownMenu anchored to the More button.
 */
...
import { DETAIL_LIST_W } from "./desktop-detail-list";

export function WLDesktopDetailMenu() {
  const sel = WORDS[0];
  return (
    <DesktopShell
      active="library"
    ...
    >
      <div style={{ flex: 1, display: "flex", minHeight: 0, minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflow: "auto", minHeight: 0, background: C.surface.page }}>
          <WordDetail w={sel} scope="desktop" menuOpen />
        </div>
      </div>
    </DesktopShell>
  );
